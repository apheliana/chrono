...
describe('ChronoList', () => {
  it('should fail if id is null or undefined', () => {
    expect(() => new ChronoList(null, 'name')).toThrowError('Invalid argument');
    expect(() => new ChronoList(undefined, 'name')).toThrowError('Invalid argument');
  });

  it('should fail if name is empty string, empty space, null or undefined', () => {
    expect(() => new ChronoList(0, '')).toThrowError('Invalid argument');
    expect(() => new ChronoList(0, ' ')).toThrowError('Invalid argument');
    expect(() => new ChronoList(0, null)).toThrowError('Invalid argument');
    expect(() => new ChronoList(0, undefined)).toThrowError('Invalid argument');
  });

  it('should create a list with a valid constructor', () => {
    const list = new ChronoList(0, 'name');
  ...
  it('should trim description', () => {
    const list = new ChronoList(0, 'name', ' desc ');
